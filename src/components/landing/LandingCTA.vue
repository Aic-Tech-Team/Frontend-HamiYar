...
<template>
  <section class="w-full py-12 lg:py-20 flex-center container-padding">
    ...
    <!-- Changed background to brand-primary-600 to make the whole banner brighter! -->
    <div 
    ...
    >
      ...
      <!-- MAGIC TRICK: The Brighter Glowing Corner Circles -->
      <!-- We used brand-primary-300 and boosted the opacity to /70 so they are highly visible! -->
      <div class="glow-circle-tr absolute -top-24 -right-24 size-72 md:size-96 rounded-full bg-brand-primary-300/70 blur-[50px] md:blur-[70px] pointer-events-none"></div>
      
      <div class="glow-circle-bl absolute -bottom-28 -left-28 size-80 md:size-[28rem] rounded-full bg-brand-primary-400/60 blur-[60px] md:blur-[80px] pointer-events-none"></div>
      ...
      <div class="relative z-10 flex flex-col items-center">
        ...
        <!-- Subtitle -->
        <p class="text-white/80 text-sm sm:text-base md:text-lg max-w-2xl mb-8 md:mb-12 leading-relaxed">
          به بیش از ۵۰,۰۰۰ دانشجوی فعال در سامانه حامی یار بپیوندید و تجربه ای نوین داشته باشید.
        </p>

        <!-- The Shadcn Button -->
        <RouterLink to="/login">
          <!-- Look at the custom button shadow added below! -->
          <Button 
            size="lg" 
...
<style scoped>
/* Subtle breathing animation for the background lights */
@keyframes pulseGlow {
  0%, 100% {
    transform: scale(1);
    opacity: 0.7;
  ...
  50% {
    transform: scale(1.2);
    opacity: 0.9; /* Makes them pulsate into a brighter glow */
  }
}
...
/* Banner Container Shadow & Hover Adjustments */
.cta-banner {
  /* Using your primary color for a deep, smooth box-shadow glow */
  box-shadow: 0 30px 60px -15px rgba(83, 33, 180, 0.4);
...
@media (min-width: 1024px) {
  .cta-banner:hover {
    ...
    box-shadow: 0 40px 80px -20px rgba(83, 33, 180, 0.55);
  }
}
</style>